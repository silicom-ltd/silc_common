...
uint32_t silc_htbl_default_cstr_hash(silc_cstr p_str)
{
	uint64_t r = silc_htbl_jenkins_hash((unsigned char*)p_str, strlen(p_str));
	return r;
}

int silc_htbl_default_cstr_comp(const silc_cstr p_str1, const silc_cstr p_str2)
{
...
#define hash(k, length, level) jenkins_hash(k, length, level)
#define hash2(k, length, level) jenkins_hash2(k, length, level)
#define hash3(k, length, level) jenkins_hash3(k, length, level)
